...
void usage() {
	error(
	  "Usage: ./constraints_parser -4|-5|-6 [-dDV] -f <InnoDB page or dir> -t table.sql [-T N:M] [-b <external pages directory>]\n"
	  "  Where\n"
...
int main(int argc, char **argv) {
	...
	while ((ch = getopt(argc, argv, "t:456hdDUVf:T:b:p:o:")) != -1) {
		switch (ch) {
			case 'd':
	...
	fprintf(stderr, "' IGNORE INTO TABLE `%s` FIELDS TERMINATED BY '\\t' OPTIONALLY ENCLOSED BY '\"' LINES STARTING BY '%s\\t' ", table->name, table->name);
	int i = 0;
	int comma = 0;
	int has_set = 0;
	fprintf(stderr, "(");
