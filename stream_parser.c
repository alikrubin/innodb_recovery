...
#ifdef STREAM_PARSER_DEBUG
        DEBUG("Wrong page type = %u", page_type);
#endif
        return 0;
        }
...
#ifdef STREAM_PARSER_DEBUG
        DEBUG("Wrong part len = %u", part_len);
#endif
        return 0;
        }
...
#ifdef STREAM_PARSER_DEBUG
        DEBUG("Wrong next page_id = %u", next_page);
#endif
        return 0;
        }
#ifdef STREAM_PARSER_DEBUG
        DEBUG("All criterias are good. Checking checksum");
#endif
...
inline int valid_innodb_page(page_t* page, uint64_t block_size, off64_t* step){
    ...
    *step = 1;
...
#ifdef STREAM_PARSER_DEBUG
    DEBUG("Fil Header");
    DEBUG("\tFIL_PAGE_SPACE:                   %08lX", mach_read_from_4(page + FIL_PAGE_SPACE_OR_CHKSUM));
    DEBUG("\tFIL_PAGE_OFFSET:                  %08lX", mach_read_from_4(page + FIL_PAGE_OFFSET));
	DEBUG("\tFIL_PAGE_PREV:                    %08lX", mach_read_from_4(page + FIL_PAGE_PREV));
	DEBUG("\tFIL_PAGE_NEXT:                    %08lX", mach_read_from_4(page + FIL_PAGE_NEXT));
	DEBUG("\tFIL_PAGE_LSN:                     %08lX", mach_read_from_4(page + FIL_PAGE_LSN));
	DEBUG("\tFIL_PAGE_TYPE:                        %04lX", mach_read_from_2(page + FIL_PAGE_TYPE));
	DEBUG("\tFIL_PAGE_FILE_FLUSH_LSN:  %016lX", mach_read_from_8(page + FIL_PAGE_FILE_FLUSH_LSN));
	DEBUG("\tFIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID: %08lX", mach_read_from_4(page + FIL_PAGE_ARCH_LOG_NO_OR_SPACE_ID));
	DEBUG("\tFIL_PAGE_END_LSN_OLD_CHKSUM:      %08X", oldcsumfield);
#endif
    ...
    if(page_id == 0){
#ifdef STREAM_PARSER_DEBUG
        DEBUG("page_id can not be zero");
#endif
        return 0;
        }
    if(page_id > max_page_id){
#ifdef STREAM_PARSER_DEBUG
	    DEBUG("page_id %lu is bigger than maximum possible %lu", mach_read_from_4(page + FIL_PAGE_OFFSET), max_page_id);
        DEBUG("Invalid INDEX page");
#endif
	    return 0;
	    }
...
#ifdef STREAM_PARSER_DEBUG
    DEBUG("Page Header");
    DEBUG("\tPAGE_N_HEAP: %08X", page_n_heap);
	DEBUG("\tVersion: %s", (version == 1)? "COMPACT" : "REDUNDANT");
#endif
    if(version == 1){
    ...
    if(page[inf_offset + 0] != 'i'
            ...
            || page[inf_offset + 6] != 'm'){
#ifdef STREAM_PARSER_DEBUG
        DEBUG("infimum record is not found");
#endif
    ...
    if(page[sup_offset + 0] != 's'
            ...
            || page[sup_offset + 7] != 'm'){
#ifdef STREAM_PARSER_DEBUG
        DEBUG("supremum record is not found");
#endif
        goto invalid_innodb_page_exit;
        }
#ifdef STREAM_PARSER_DEBUG
    DEBUG("Valid INDEX page");
#endif
    return 1;
...
#ifdef STREAM_PARSER_DEBUG
	DEBUG("Old checksum: calculated=%lu, stored=%lu", oldcsum, oldcsumfield);
#endif
...
inline void process_ibpage(page_t* page){
    ...
    if(page_type == FIL_PAGE_INDEX){
        ...
        sprintf(file_name, "%s/FIL_PAGE_TYPE_BLOB/%016lu.page", dst_dir, page_id);
...
void process_ibfile(int fn, off64_t start_offset, ssize_t length){
    ...
    off64_t progress = 0, progress_prev = 0;
    off64_t step;
...
#ifdef STREAM_PARSER_DEBUG
        DEBUG("Reading from offset %lu", curr_disk_offset);
#endif
...
#ifdef STREAM_PARSER_DEBUG
        DEBUG("Read %u bytes from disk to RAM", disk_read);
#endif
...
#ifdef STREAM_PARSER_DEBUG
            DEBUG("Checking page at cache offset %lu", curr_cache_offset);
#endif
            if (0
...
#ifdef STREAM_PARSER_DEBUG
            DEBUG("Moving cache pointer %lu bytes", cache_step);
#endif
...
#ifdef STREAM_PARSER_DEBUG
            DEBUG("%lu bytes remain in the cache. Moving it to the beginning", bytes_in_cache - curr_cache_offset);
#endif
...
#ifdef STREAM_PARSER_DEBUG
        DEBUG("curr_disk_offset = %llu, start_offset = %llu", curr_disk_offset, start_offset);
#endif
...
#ifdef STREAM_PARSER_DEBUG
        DEBUG("Disk offset at the end of read cycle %llu", curr_disk_offset);
#endif
        }
    return;
}
...
int main(int argc, char **argv) {
    ...
    for(n = 0; n < ncpu; n++){
        ...
        if(pid == 0){
            ...
            //if(worker == 0) debug = 1;
#ifdef STREAM_PARSER_DEBUG
            DEBUG("I'm child(%d): %u.", n, getpid());
            DEBUG("Processing from %lu bytes starting from %lu", ib_size/ncpu, n*ib_size/ncpu);
#endif
