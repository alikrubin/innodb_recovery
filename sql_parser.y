...
field_def_t*  find_field(field_def_t *haystack, field_def_t* needle){
    field_def_t *result = NULL;
    unsigned int i;
    int same = 0;
    int size = (strlen(needle->name) > strlen(haystack->name)) ? strlen(needle->name): strlen(haystack->name);
    while((haystack + i)->name != NULL){
        same = strncmp((haystack + i)->name,
            needle->name, size);
        if(same == 0){
            return (haystack + i);
            }
	...
	opt_list_table_option ';'{
        ...
        for(i = 0; i < field_number; i++){
            ...
            if(all_fields[i].charset == NULL){
                all_fields[i].fixed_length * factor;
                }
            }
        ...
        if(has_primary_key){
            ...
            for(i = 0; i < pk_field_number; i++){
                ...
                memcpy(table_definitions[0].fields + meta_i, find_field(all_fields, pk_fields + i), sizeof(field_def_t));
                meta_i++;
                }
            }
        else{
            if(has_unique_key){
                ...
                for(i = 0; i < uq_field_number; i++){
                    ...
                    memcpy(table_definitions[0].fields + meta_i, find_field(all_fields, uq_fields + i), sizeof(field_def_t));
                    meta_i++;
                    }
        ...
        for(i = 0; i < field_number; i++){
            if(find_field(table_definitions[0].fields, all_fields + i) == 0 ){
                if(debug) printf("Other field: %s\n", all_fields[i].name);
...
create_definition: col_name 
        ...
        { 
            ...
            all_fields[field_number].has_limits = 0;
...
data_type: BIT { 
	...
	| TINYINT { 
        ...
        if(is_unsigned) all_fields[field_number].type = FT_UINT;
        }opt_zero_fill
	| SMALLINT { 
        all_fields[field_number].type = FT_INT;
...
collate_clause: COLLATE entity_name;
